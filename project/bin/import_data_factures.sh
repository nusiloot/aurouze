...
cat  $DATA_DIR/tblFacture.csv | tr "\r" '~' | tr "\n" '#' | grep -Ev "^RefFacture;Numero;" | sed -r 's/~#([0-9]+;[0-9]+;[0-1]{1};)/\n\1/g' | sed -r 's/~#/#/g'  | sed -r 's/([a-zA-Z]+)[ ]+([0-9]+)[ ]+([0-9]+)[ ]+([0-9:]+):[0-9]{3}([A-Z]{2})/\1 \2 \3 \4 \5/g' | sort -t ";" -k 8,8 > $DATA_DIR/factures.cleaned.sorted.csv
...
cat $DATA_DIR/tblFactureLigne.csv | tr "\r" '~' | tr "\n" '#' | sed -r 's/~#([0-9]+;[0-9]+;)/\n\1/g' | sed -r 's/~#/#/g' | sed -r 's/([a-zA-Z]+)[ ]+([0-9]+)[ ]+([0-9]+)[ ]+([0-9:]+):[0-9]{3}([A-Z]{2})/\1 \2 \3 \4 \5/g' | sort -t ";" -k 2,2 > $DATA_DIR/factureslignes.cleaned.sorted.csv
